...
midcol = config_vars["module_options"]["P2"]["col2_name"]
...
for i in sources:
    ...
    split_source2 = [i.split(sep = "__") for i in split_source]
...
for key in to_test_graph:
    ...
    agg_start_mini = agg_start_mini.assign(type = "start")
    agg_start_mini = agg_start_mini.rename(columns={"start":"loci", "start_peak":"peak"})

    agg_end_mini = agg_end_mini.assign(type = "end")
    ...
    for row in peaks_dataframe.iterrows():
        ...
        # How long is the length of the cluster => Create minimum
        ...
        if type_loci == 'start':
            ...
            if naming_start_cond:
                if peak_status:
                    ...
                    # only progress if cluster minimum is reached
                    if len_of_clust >= cluster_min:
                        current_id += 1
    ...
    if i_1 == 0:
        ...
        the_peaker = the_peaker.assign(source = key)
        ...
        the_peaker_temp = the_peaker_temp.assign(source = key)
...
for i in S:
    ...
    source_finder = list(i.edges(data=True))[0]
    
    if prefix in source_finder[0]:
        source_name = source_finder[1]
    else:
        source_name = source_finder[0]
...
if make_lots == True:
    if len(alt_peaks_df["start_end"]) == 0:
        for source in list(np.unique(the_peaker["source"])):
            ...
            source_alt = source.replace("/", '.')

            plt.legend()
            plt.savefig("cluster/cluster_loc_" + source_alt + ".jpeg")
            plt.clf()
        ...
        for source in list(np.unique(the_peaker["source"])):
            ...
            starts_alt = [int(p) for p in starts_alt]
            ends_alt = [int(p) for p in ends_alt]
            ...
            for n in list(np.unique(source_specific_df["cluster"])):
                ...
                if i in starts_alt:
                    num_add_start = starts_alt.count(i)
                    
                    for repeat in range(0, num_add_start):
                        alt_starts.append(min(x_constructor))
                if i in ends_alt:
                    num_add_end = ends_alt.count(i)
                
                    for repeat in range(0, num_add_end):
